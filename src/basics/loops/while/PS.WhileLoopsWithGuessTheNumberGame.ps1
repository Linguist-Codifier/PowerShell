...
[String]$DefaultMessage = 'Guess the secret number';

[String]$DefaultMessageOnError = 'Not yet'; 

[Boolean]$DefaultMessageWasCalled = $false;

try { while(([Int32]$Buffer = Read-Host -Prompt $DefaultMessage) -ne $SecretNumber) {
        if($DefaultMessageWasCalled -eq $false) {
...
catch { Write-Host -ForegroundColor Yellow 'It seems you do not want to play the game,' -NoNewline;
            Write-Host -ForegroundColor Yellow ' as it needs to be a number. So please try again.';
}

finally { if($Buffer -eq $SecretNumber) {
